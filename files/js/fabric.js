  ...
  fabric.StaticCanvas = fabric.util.createClass(/** @lends fabric.StaticCanvas.prototype */ {
    ...
    _renderOverlay: function(ctx) {
      ...
      if (this.overlayImage) {
		  this.overlayImage.width=CraPacrAxx.getWidth();
		 this.overlayImage.height=CraPacrAxx.getHeight();
		 this.overlayImage.left=0;
		 this.overlayImage.top=0;
  ...
  if (global.fabric.Imagepattern) {
   ...
   fabric.Imagepattern = fabric.util.createClass(fabric.Object, /** @lends fabric.Imagepattern.prototype */ {
	...
	//imagePlace = this._setImgInBox(),
		...
		if (this.qualit!=0 && quality_ratio!=4 && !is_finish) {
			
			
			var qual_max,qual_min;
			...
			var qualit_actuell = (this.qualit * (((this.widthPat*this.scaleImg) * (this.heightPat*this.scaleImg) * 100 )/(this.widthImg* this.heightImg )))/100;
			
			var pourcent_canv = (this.width* this.height * 100 )/(CraPacrAxx.width*CraPacrAxx.height) ;
			
			var lst_tail = liste_taille.split('--sepracanv--')[id_obj_sel].split(',');
			
			if (lst_tail[2]!='mm') var qualit_canvas = (lst_tail[0]*2.54)*(lst_tail[1]*2.54)*118*118;
			else var qualit_canvas = (lst_tail[0]/10)*(lst_tail[1]/10)*118*118;
			var qualit_necess = (qualit_canvas * pourcent_canv)/100;
			
			
			
			this.qualit_flag = 0;
			if (qualit_actuell<(qualit_necess/qual_max)) {
				var source = new Image();				
				if (qualit_actuell<(qualit_necess/qual_min)) {
					source.src = IMAGE_INTERF+'warning_low.svg';
					this.qualit_flag = 1;
				} else  source.src = IMAGE_INTERF+'warning_median.svg';
					
				ctx.save();
			//	ctx.moveTo((-3*(this.width)),(-3*(this.height)));
				
				ctx.drawImage(source,(-18),(-0.4*this.height),36,36);
				if (this.width> 180 && this.height> 180) {
					ctx.lineWidth=1;			
					ctx.font="20px Verdana";
					ctx.fillStyle='#FFFFFF';
					ctx.strokeStyle='#000000';
					ctx.fillText(quality_texte,-40,(-0.4*this.height)+60);
					ctx.strokeText(quality_texte,-40,(-0.4*this.height)+60); 
				}
				ctx.restore();
			} 
